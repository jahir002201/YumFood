...
def initiate_payment(request):
    print("Initiate payment", request.data, request.user)
    user = request.user
...
def payment_success(request):
    ...
    try:
        ...
        order.save()
        Payment.objects.filter(transaction_id=tran_id).update(status=Payment.SUCCESS)
...
def payment_fail(request):
    tran_id = request.data.get("tran_id")
    if tran_id:
        Payment.objects.filter(transaction_id=tran_id).update(status=Payment.FAILED)
    return HttpResponseRedirect(f"{main_settings.FRONTEND_URL}/dashboard/orders/")
